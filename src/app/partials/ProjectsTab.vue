...
const getImgSrc = (path) => new URL(`/src/assets/images/${path}`, import.meta.url).href;
</script>

<template>
    <section class="wm-projects-tab">
        <article
            v-for="{ title, link, logoPath, description, stack } in config"
            :key="`project: ${title}`"
            class="wm-project-card"
        >
            <aside>
                <img
                    :src="getImgSrc(logoPath)"
                    :alt="`${title} logo`"
                />
            </aside>
...
.wm-project-card {
    ...
    & > aside {
        ...
        @include block-UI(true, true, (4px 0 0 4px));
    ...
    &__wrapper {
        ...
        & > header {
            ...
            padding: 3px 3px 3px $space--half;
